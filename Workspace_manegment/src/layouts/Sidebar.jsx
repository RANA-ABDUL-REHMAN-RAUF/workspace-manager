...
import { Activity, FolderKanban, Home, Inbox, LayoutGrid, Plus, Search, Settings, HelpCircle, UserPlus, X, CheckSquare, Users, ChevronDown, PanelLeftClose, PanelLeftOpen } from 'lucide-react'
...
const itemClass = ({ isActive }) => `flex items-center gap-2.5 rounded-md px-2.5 py-2 text-[11px] transition-colors ${isActive ? 'bg-[#eeeaff] font-medium text-[#8272d4]' : 'text-[#8990a3] hover:bg-[#f7f6fd] dark:text-slate-400 dark:hover:bg-slate-800'}`
...
export default function Sidebar({ drawerOpen, onCloseDrawer, onCreateProject }) {
  ...
  return <>
    ...
    <aside className={`fixed inset-y-0 left-0 z-30 flex w-56 shrink-0 flex-col overflow-y-auto border-r border-[#eaecf4] bg-white transition-transform md:sticky md:top-0 md:z-auto md:h-svh md:translate-x-0 dark:border-slate-800 dark:bg-slate-900 ${collapsed ? 'md:w-[72px]' : 'md:w-[224px]'} ${drawerOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="relative flex h-[57px] shrink-0 items-center gap-2 border-b border-[#f0f1f7] px-4 dark:border-slate-800">
        <button onClick={() => setMenu(!menu)} aria-label="Switch workspace" aria-expanded={menu} className="flex min-w-0 flex-1 items-center gap-2"><span className="grid size-6 shrink-0 place-items-center rounded-md bg-[#7e71df] text-[9px] font-semibold text-white">{workspace?.icon || 'W'}</span><span className={`truncate text-[11px] font-semibold ${collapsed ? 'md:hidden' : ''}`}>{workspace?.name || 'Workspaces'}</span><span className={`rounded bg-violet-50 px-1 text-[7px] text-violet-400 ${collapsed ? 'md:hidden' : ''}`}>PRO</span><ChevronDown size={10} className={`ml-auto text-slate-400 ${collapsed ? 'md:hidden' : ''}`} /></button>
      ...
      <div className="flex flex-1 flex-col px-2.5 py-3">
        <nav aria-label="Main navigation" className="space-y-0.5">{PRIMARY.map(([Icon, label, to]) => <NavLink key={to} to={to} title={label} onClick={onCloseDrawer} className={itemClass}><Icon size={14} strokeWidth={1.6} /><span className={`flex-1 ${collapsed ? 'md:hidden' : ''}`}>{label}</span>{(label === 'My Tasks' ? myTasksCount : label === 'Inbox' ? inboxCount : 0) > 0 && <span className={`rounded bg-[#efedff] px-1.5 text-[8px] text-violet-500 ${collapsed ? 'md:hidden' : ''}`}>{label === 'My Tasks' ? myTasksCount : inboxCount}</span>}</NavLink>)}</nav>
        {workspace && <><p className={`mb-1 mt-5 px-2.5 text-[8px] tracking-[1px] text-[#b0b5c5] ${collapsed ? 'md:hidden' : ''}`}>WORKSPACE</p><nav aria-label="Workspace navigation" className="space-y-0.5">{workspaceNav.map(([Icon, label, to]) => <NavLink end key={to} to={to} title={label} onClick={onCloseDrawer} className={itemClass}><Icon size={14} strokeWidth={1.6} /><span className={collapsed ? 'md:hidden' : ''}>{label}</span></NavLink>)}</nav>
        <div className={`mt-5 mb-1 flex items-center justify-between px-2.5 ${collapsed ? 'md:hidden' : ''}`}><p className="text-[8px] tracking-[1px] text-[#b0b5c5]">PROJECTS</p><button disabled={!manageable} onClick={onCreateProject} aria-label="New project" className="text-slate-400 disabled:opacity-30"><Plus size={12} /></button></div><nav aria-label="Projects" className="space-y-0.5">{projects.filter(p => !p.archived).map(project => <NavLink key={project.id} title={project.name} to={generatePath(ROUTES.project, { workspaceId: workspace.id, projectId: project.id })} onClick={onCloseDrawer} className={itemClass}><span className="size-1.5 shrink-0 rounded-sm bg-[#a193e2]" /><span className={`truncate ${collapsed ? 'md:hidden' : ''}`}>{project.name}</span></NavLink>)}</nav></>}
        <div className="mt-auto space-y-0.5 pt-12"><button disabled={!manageable} onClick={() => setInviteOpen(true)} title="Invite members" className="flex w-full items-center gap-2.5 rounded-md px-2.5 py-2 text-[11px] text-[#8990a3] hover:bg-slate-50 disabled:opacity-40"><UserPlus size={14} /><span className={collapsed ? 'md:hidden' : ''}>Invite Members</span></button><NavLink to={ROUTES.settings} title="Settings" onClick={onCloseDrawer} className={itemClass}><Settings size={14} /><span className={collapsed ? 'md:hidden' : ''}>Settings</span></NavLink><button title="Help and shortcuts" onClick={() => setHelp(true)} className="flex w-full items-center gap-2.5 rounded-md px-2.5 py-2 text-[11px] text-[#8990a3] hover:bg-slate-50"><HelpCircle size={14} /><span className={collapsed ? 'md:hidden' : ''}>Help & Docs</span></button></div>
        <div className="mt-3 flex items-center gap-2 border-t border-[#eef0f6] px-1 pt-3 dark:border-slate-800"><Link to={ROUTES.profile} title="Your profile"><Avatar user={session} small /></Link><div className={`min-w-0 flex-1 ${collapsed ? 'md:hidden' : ''}`}><p className="truncate text-[10px] font-medium">{session?.name}</p><p className="truncate text-[8px] text-slate-400">{session?.email}</p></div><button title={collapsed ? 'Expand sidebar' : 'Collapse sidebar'} aria-label={collapsed ? 'Expand sidebar' : 'Collapse sidebar'} onClick={() => setCollapsed(!collapsed)} className="hidden text-slate-400 md:block">{collapsed ? <PanelLeftOpen size={13} /> : <PanelLeftClose size={13} />}</button></div>
      </div>
    </aside>
