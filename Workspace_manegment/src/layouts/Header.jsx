...
export default function Header({ onOpenDrawer, onOpenPalette }) {
  ...
  return <header className="sticky top-0 z-10 flex min-h-[57px] flex-wrap items-center gap-3 border-b border-[#eaecf4] bg-white px-4 py-2 sm:px-6 lg:px-7 dark:border-slate-800 dark:bg-slate-900">
    ...
    <div className="relative">
      <button type="button" onClick={() => setWorkspaceMenuOpen((open) => !open)} className="flex items-center gap-1 text-[10px] font-medium text-slate-400">
        {workspace?.name || 'Workspace'} <ChevronDown size={14} className="text-slate-400" />
      </button>
      {workspaceMenuOpen && <div className="absolute left-0 z-20 mt-2 w-56 rounded-md border border-slate-200 bg-white p-1 shadow-lg">
    ...
    <span className="hidden text-[10px] text-slate-400 sm:inline">/ <span className="ml-2 text-[#7764cd]">{project?.name || 'Workspace overview'}</span></span>

    <span className={`hidden items-center gap-1 rounded border px-1.5 py-0.5 text-[8px] sm:inline-flex ${!online ? 'border-red-100 bg-red-50 text-red-600' : syncing ? 'border-amber-100 bg-amber-50 text-amber-700' : 'border-slate-100 bg-slate-50 text-slate-400'}`}>
      {!online && <WifiOff size={11} />}{!online ? 'Offline' : syncing ? 'Syncing…' : 'Synced'}
    </span>

    <div className="ml-auto flex items-center gap-2">
      <Button disabled={!manageable} onClick={() => setInviteOpen(true)} className="hidden text-[10px]! sm:inline-flex">Share</Button>
      <button type="button" onClick={onOpenPalette} className="flex items-center gap-2 rounded-md border border-slate-100 px-2 py-1.5 text-[10px] text-slate-400 hover:bg-slate-50">
        <Search size={15} /><span className="hidden sm:inline">Search</span><span className="rounded border border-slate-200 px-1 text-[10px]">⌘K</span>
      </button>

      <div className="hidden -space-x-2 sm:flex">
        {members.slice(0, 3).map((u) => <Avatar key={u.id} user={u} small />)}
        {members.length > 3 && <span className="grid size-6 place-items-center rounded-full bg-slate-100 text-[9px] font-medium text-slate-500">+{members.length - 3}</span>}
      </div>

      <div className="relative">
        <button type="button" onClick={() => setNotifOpen((open) => !open)} aria-label="Notifications" className="relative rounded-md p-2 hover:bg-slate-100">
          <Bell size={18} />
          {unread > 0 && <span className="absolute -top-0.5 -right-0.5 size-2 rounded-full bg-red-500" />}
        </button>
        {notifOpen && <div className="absolute right-0 z-20 mt-2 w-80 rounded-md border border-slate-200 bg-white shadow-lg">
          ...
          <ul className="max-h-80 overflow-auto">
            {notifications.map((n) => <li key={n.id} className={`border-b border-slate-50 p-3 text-xs last:border-0 ${n.read ? 'text-slate-400' : 'text-slate-700'}`}>
              <button className="text-left" onClick={() => { dispatch(actions.readNotification(n.id)); const task = data.tasks.find(t => t.id === n.taskId); const project = data.projects.find(p => p.id === task?.projectId); if (project) { navigate(`/workspaces/${project.workspaceId}/projects/${project.id}/tasks/${task.id}`); setNotifOpen(false) } }}>{n.text}</button><p className="mt-0.5 text-[10px] text-slate-400">{formatRelativeTime(n.at)}</p>
            </li>)}
      ...
      <div className="relative">
        <button type="button" onClick={() => setAccountOpen((open) => !open)} aria-label="Account menu"><Avatar user={session} /></button>
        {accountOpen && <div className="absolute right-0 z-20 mt-2 w-64 rounded-md border border-slate-200 bg-white p-1 shadow-lg">
          ...
          <p className="px-3 pt-2 pb-1 text-[10px] font-semibold tracking-wider text-slate-400">SWITCH PROFILE</p>
